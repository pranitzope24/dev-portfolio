      ...
      <div className="p-6">
        <div className="grid grid-cols-2 gap-4">
          {hobbies.map((h, idx) => (
            <div
              key={h.label}
              className="group flex items-center gap-3 rounded-xl2 border border-border/60 bg-bg/20 px-4 py-3"
              title={!simpleView ? `${h.label} — ${h.hint}` : h.label}
            >
              <div
                className={`grid h-9 w-9 place-items-center rounded-xl border border-border/60 bg-panel/35 ${
